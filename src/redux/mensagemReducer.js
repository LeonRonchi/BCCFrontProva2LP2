...
import STATE from "../recursos/estado.js";
...
export const buscaMensagem = createAsyncThunk("message/get", async () => {
    try {
        const response = await fetch("https://backend-bcc-2-b.vercel.app" + "/mensagem", {
            method: "GET",
            headers: {
...
const messageSlice = createSlice({
    ...
    extraReducers: (builder) => {
        builder.addCase(buscaMensagem.pending, (state, action) => {
            state.state = STATE.PENDENTE;
            state.mensagem = "Buscando mensagens...";
        })
        builder.addCase(buscaMensagem.fulfilled, (state, action) => {
        ...
        builder.addCase(buscaMensagem.rejected, (state, action) => {
            state.state = STATE.ERRO;
